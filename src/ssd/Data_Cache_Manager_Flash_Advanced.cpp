			...
			case Caching_Mode::WRITE_READ_CACHE:
			{
				...
				while (it != user_request->Transaction_list.end())
				{
					...
					if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA))//逻辑地址存在cache中
					{
						...
						//Data_Cache_counter.data_cache_read_hit++;
						if (available_sectors_bitmap == tr->read_sectors_bitmap)//全命中
						{
		...
		else//This is a write request
		{
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					if (shared_dram_request_queue)//bo0l shared_dram_request_queue 逻辑判断如果是共享队列的情况下
						queue_id = 0;
